...
permalink: /pages/v2.11.X/c0f5d7/
---

在实际业务中，往往要通过动态的业务逻辑判断到底接下去该执行哪一个节点，这就引申出了选择节点，选择节点可以用于`SWITCH`关键字中。

关于SWITCH表达式的用法，可以参考[选择编排](/pages/v2.11.X/d90483/)一章。
...
关于tag(标签)的概念，请参照[组件标签](/pages/v2.11.X/0f788f/)这一章。
...
选择节点的内部可以覆盖的方法和this关键字可调用的方法见[普通组件](/pages/v2.11.X/8486fb/)这一章。
