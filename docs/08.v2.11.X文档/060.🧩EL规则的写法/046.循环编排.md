...
其中f这个节点需要为次数循环组件，返回一个int循环次数，如何定义请参照[次数循环组件](/pages/master/5f971f/)。
...
其中w这个节点需要为条件循环组件，返回一个布尔值，为true则继续循环，如何定义请参照[条件循环组件](/pages/master/e6995e/)。
...
其中x这个节点需要为迭代循环组件，返回一个迭代器，如何定义请参照[迭代循环组件](/pages/master/64262b/)。
...
其中c这个节点需要为退出循环组件，返回一个布尔值，为true则退出循环。如何定义请参考[退出循环组件](/pages/master/c078cd/)。
...
LiteFlow从v2.11.0支持了异步循环特性，关于异步循环请参考高级特性中的[异步循环模式](/pages/master/35cc4a/)。
