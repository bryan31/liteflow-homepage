...
新版本中，`WHEN` 关键字新增子关键字 `must` (不可为空)，可用于指定需等待执行的任意节点，可以为 1 个或者多个，若指定的所有节点率先完成，则继续往下执行，忽略同级别的其他任务，用法如下：

```xml
<chain name="chain1">
    THEN(
        a,
        WHEN(b, THEN(c, d).id("z"), e).must(b, "z"),
        f
    );
</chain>
```

以上流程中，指定了 b 节点以及 id 为 z 的 `THEN` 子流程，那么流程会等待它们至完成，方才会继续执行后续的 f 节点。值得一提的是，`must` 关键字是根据 id 去判断获取对应的任务，故而在使用关键字时，需要注意区分，可填写对应的节点，亦或者填写指定字符串类型的 id。
