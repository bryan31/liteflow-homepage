...
但从 2.16.1 起，我们**更推荐使用全新的 [Rule-DB 模式](/pages/ruledb-overview/)**（本例对应 `liteflow-rule-db-etcd`，见 [liteflow-rule-db-etcd 文档](/pages/ruledb-etcd/)）。它把存储作为规则的权威源，多节点最终一致、秒级收敛，规则正文按热度缓存而非全量常驻堆内存。
