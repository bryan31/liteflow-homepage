...
- 在 `drawFrame` 每帧调用 `isDarkTheme()` 读取 `document.documentElement.classList` 实现主题感知
- 检测 `theme-mode-dark`、`theme-mode-read`（两者使用相同的暗色配色），否则视为亮色模式
- 主题切换时颜色参数在下一帧自动更新，无需重建节点
- **注：** 原设计方案为 MutationObserver，实现中改为每帧轮询，功能等价且无需管理 observer 生命周期，已确认为有意设计决策
