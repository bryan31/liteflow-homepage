...
9. **开启 Skills 等于开启代码执行**：开启 skills 后，框架会在 conversation workspace 内启用 AgentScope 的代码执行能力。这条路径**独立于 `liteflow.agent.shell.*`**，即使 Shell 设为 `DISABLED` 也不受白名单约束。开启前应评估该路径，并严格管控 `skills.path` 与其引用的 Java 工具类来源（详见 [Skills 技能系统](/pages/agent-skills/)）。
