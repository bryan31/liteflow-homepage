...
这一节以及后续存储、工作区配置主要用于对话 Agent。独立的 Jev 选择组件使用本页“Jev 选择组件”一节的配置，不要求配置会话或工作区。
...
这些属性只在使用 `liteflow-agent-jev` 时生效，与聊天模型平台凭据独立。

| 配置 | 默认值 | 用途 |
| --- | --- | --- |
| `jev.provider` | `typesafe` | 支持 `typesafe`、`openrouter`；未知值在 Spring Boot 属性绑定时失败 |
| `jev.api-key` | 未设置 | 使用 Jev 时必填，填写所选 provider 的凭据；其他组件不需要填写 |
| `jev.base-url` | 随 provider 选择 | `typesafe`：`https://api.typesafe.ai/v1`，追加 `/systemone`；`openrouter`：`https://openrouter.ai/api/alpha`，追加 `/decisions` |
| `jev.model` | 随 provider 选择 | `typesafe`：`jev-1.13.0`；`openrouter`：`typesafe/jev-1.13` |
| `jev.timeout` | `3s` | 完整 HTTP 响应的等待时限，包含响应体读取，至少 `1ms` |
| `jev.min-confidence` | `0.6` | 0～1，低于此值进入 `DEFAULT`；组件可通过 `minConfidence()` 覆写 |

`base-url`、`model` 未设置或为空白时才使用 provider 默认值，显式配置不会随 provider 切换而重置。`base-url` 填 API 根地址，不含 `/systemone` 或 `/decisions` 接口路径；OpenRouter 使用 Decisions API，不使用聊天接口。

对应配置类为 `com.yomahub.liteflow.property.agent.JevConfig`，通过 `AgentConfig#getJev()` 获取；Java 中使用 `JevProvider.TYPESAFE` 或 `JevProvider.OPENROUTER`。置信度与选项概率不同，阈值需要按业务数据校准。使用方式见 [Jev 依赖与配置](/pages/agent-jev-config/)。
