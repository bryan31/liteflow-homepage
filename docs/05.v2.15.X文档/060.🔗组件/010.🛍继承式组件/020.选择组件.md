...
permalink: /pages/v2.15.X/c0f5d7/
---

在实际业务中，往往要通过动态的业务逻辑判断到底接下去该执行哪一个节点，这就引申出了选择节点，选择节点可以用于`SWITCH`关键字中。

关于SWITCH表达式的用法，可以参考[选择编排](/pages/v2.15.X/d90483/)一章。
...
关于tag(标签)的概念，请参照[tag语法](/pages/v2.15.X/cc24b8/)这一章。
...
选择节点的内部可以覆盖的方法和this关键字可调用的方法见[组件内方法覆盖和调用](/pages/v2.15.X/83073e/)这一章。
...
## 拿到当前Target节点列表<Badge text="v2.15.0+"/>

你可以在选择组件中通过`this.getTargetList()`来拿到当前目标节点的Id列表。