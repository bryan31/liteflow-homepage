...
permalink: /pages/v2.15.X/83073e/
---
...
用于执行一些前置和后置处理，但是一般这个用不上。如果是统一做组件前置和后置，推荐用切面去完成。关于切面可以参考[组件切面](/pages/v2.15.X/2373f5/);

### onSuccess和onError

流程的成功失败事件回调，详情请参考[组件事件回调](/pages/v2.15.X/3ee755/)。



### rollback
流程失败后的回滚方法，详情请参考[组件回滚](/pages/v2.15.X/y172l7/)。
...
获取这个组件的标签信息，关于标签的定义和使用，请参照[tag语法](/pages/v2.15.X/cc24b8/)。

### invoke和invoke2Response

调用隐式流程，关于隐式流程的说明和用法，请参考[隐式子流程](/pages/v2.15.X/80e873/)。
