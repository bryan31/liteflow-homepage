...
新增 `liteflow-agent-jev` 模块，通过 `JevSwitchComponent` 接入 Jev Choice，按自然语言选择组件、Agent 或子链，支持置信度阈值、`DEFAULT` 兜底与决策回调。`liteflow.agent.jev.provider` 支持 `typesafe`（默认）与 `openrouter`，按入口提供默认地址与模型。详见 [Jev 智能选择组件](/pages/agent-jev-switch/)。

提供独立的 [Jev 客服分流演示](/pages/agent-jev-example/)，可查看执行分支、置信度、候选概率与处理时间。

Agent 模块统一使用 `liteflow-agent-*` 命名，对话与工具调用组件通过 `HarnessAgentComponent` 接入 EL 编排。

提供 JSON、Redis、MySQL 会话存储，保存续聊状态、聊天历史与工作区数据，并支持会话查询、分页和删除。

支持本地与 Docker 命令执行、工作区同步、容器快照与会话级容器复用。

完善 Java 工具、MCP、Skills、流式事件、结构化输出、长对话压缩和长期记忆能力。

支持工具调用前人工确认、子代理与计划模式，以及远程 A2A Agent 调用。

已有 Agent 项目需同步调整依赖、组件 API 和配置。详见 [2.16.2 版本说明](/pages/8ff02b/)与 [Agent 升级说明](/pages/agent-upgrade-2-16-2/)。
...
特性 全新的 Rule-DB 规则存储模式，存储作为规则权威源，多节点最终一致、秒级收敛，规则正文按热度缓存

提供七个后端：liteflow-rule-db-sql（MySQL/MariaDB）、liteflow-rule-db-postgresql、liteflow-rule-db-mongodb、liteflow-rule-db-redis、liteflow-rule-db-zk、liteflow-rule-db-etcd、liteflow-rule-db-nacos

特性 七个 Rule-DB 后端共用一套统一发布 API（RulePublisher），支持 expectedVersion 乐观锁，可独立用于管理后台

特性 新增 liteflow-metrics 模块，内置 Micrometer 埋点，chain / node 的执行次数、耗时、错误率、在途执行数可直接对接 Prometheus 与 Grafana

特性 新增 /actuator/liteflow 结构端点，可查看 chain / node 定义信息与指标快照，含未执行过的 chain / node

特性 新增 /actuator/liteflow/ruledb 端点，暴露 Rule-DB 同步水位、目标状态计数与加载失败明细

增强 旧的 6 个 liteflow-rule-* 规则配置源插件保持完整可用并继续维护，新项目推荐使用 Rule-DB 模式
...
特性 #IJJAB4 LiteFlow 2.16.0 推出Reasoning Action 的大模块，可以参与编排

https://gitee.com/dromara/liteFlow/issues/IJJAB4

特性 #IHWU99 支持Spring Boot 4

https://gitee.com/dromara/liteFlow/issues/IHWU99

增强 #IJDT0U Java25支持js脚本

https://gitee.com/dromara/liteFlow/issues/IJDT0U

增强 #IDMC8I 2.15.3版本执行java脚本提示script for node is not loaded

https://gitee.com/dromara/liteFlow/issues/IDMC8I

修复 #IDO5JK java脚本不能热更新

https://gitee.com/dromara/liteFlow/issues/IDO5JK

修复 #IDP551 validateWithEx这个方法在校验有子流程的EL时会报NPE

https://gitee.com/dromara/liteFlow/issues/IDP551

修复 #IIYPFL 修复 PercentageOfParallelExecutor 并行任务执行时可能出现的阻塞问题

https://gitee.com/dromara/liteFlow/issues/IIYPFL
...
增强 #IDGGGC Java脚本启动时批量编译，加快启动速度

https://gitee.com/dromara/liteFlow/issues/IDGGGC

修复 #IDCBQ2 liteflowV2.15.x bindData出现数据错乱，会固定使用第一次执行的chain的相同的bindData

https://gitee.com/dromara/liteFlow/issues/IDCBQ2

修复 #ID7OTO bind对象为chain时，chain的定义顺序影响了bind数据的获取
...
增强 #ID8XF9 对QLExpress4的支持

https://gitee.com/dromara/liteFlow/issues/ID8XF9
