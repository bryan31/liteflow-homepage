...
permalink: /pages/v2.13.X/fbf715/
---
...
其中f这个节点需要为次数循环组件，返回一个int循环次数，如何定义请参照[次数循环组件](/pages/v2.13.X/5f971f/)。
...
其中w这个节点需要为`布尔组件`，返回一个布尔值，为true则继续循环，如何定义请参照[布尔组件](/pages/v2.13.X/cb0b59/)。
...
其中x这个节点需要为迭代循环组件，返回一个迭代器，如何定义请参照[迭代循环组件](/pages/v2.13.X/64262b/)。
...
其中c这个节点需要为`布尔组件`，返回一个布尔值，为true则退出循环。如何定义请参考[布尔组件](/pages/v2.13.X/cb0b59/)。
...
具体内容请参照[脚本与Java进行交互](/pages/v2.13.X/d861c8/)中的`元数据获取方式之二`。

## 异步循环<Badge text="v2.11.0+"/>

LiteFlow支持了异步循环特性，关于异步循环请参考高级特性中的[异步循环模式](/pages/v2.13.X/35cc4a/)。
